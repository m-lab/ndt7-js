...
test('Basic functionality tests', async t => {
    // The assertion timeout is set to 5 seconds, so notEql() will wait until
    ...
    await t.expect(server.innerText).notEql('', { timeout: 5000 });

    // We're in the middle of the download test, so these divs should be
    // populated. A 5-second timeout is used here as well, to handle cases
    // where connecting to the server takes longer than usual.
    await t.expect(downloadStatus.innerText).eql('measuring', { timeout: 5000 });
    ...
    // The same 5-second timeout as above is used for the client to connect
    // for the upload test.
    await t.expect(uploadStatus.innerText).eql('measuring', { timeout: 5000 });
